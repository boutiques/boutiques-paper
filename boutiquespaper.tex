...
\abstract{Porting applications to execution platforms such as web
  ...
  environments such as Docker and Singularity to install applications
...
or directly on the command line. Such formal descriptions, simply referred to
as \emph{descriptors}, allow for advanced validation of input values to
prevent errors. A descriptor refers to a container where a given application
...
of \boutiques descriptors. \boutiques descriptors are intented to be produced by
...
Applications are described with a JSON descriptor that specifies the
command-line template, inputs and outputs. The descriptor may point to a
...
runtime, the execution platform builds the command line from the descriptor
...
schema is stored as an optional property of the \boutiques descriptor.
...
descriptors to the \boutiques schema using a basic JSON validator. It also
...
also create descriptors from select application collections, such as BIDS
...
\note{sorina}{@tristan: some info on workflows is available in the paragraph above. Is it OK/enough or do you have other suggestions?}
...
A few other applications from the FLI (France Life Imaging) consortium were also imported in
VIP using \boutiques. One of them is Ginfizz, a FMRI pipeline allowing to execute multiple 
...
greatly simplifies its deployment. The animaN4BiasCorrection tool from the
Anima\footnote{\url{https://github.com/Inria-Visages/Anima-Public/wiki}} project was also made available 
in VIP. The import process was straightforward. 

\note{sorina}{@tristan: please check if you agree with the descriptions above}
...
%%  \end{tabular} 
%%  \caption{Applications available in Boutiques with descriptor features used.}
